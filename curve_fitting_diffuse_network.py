...
from diffuse_network import tipping
...
class CurveFitting:
    ...
    def calculate_fitness(self, chromosome_base: list[int], chromosome: list[int]):
        ...
        for j in range(0, 1000):
            x_value = j / 10
            desired_y = calculate_y(x_value, chromosome_base)


            tipping.input['x'] = x_value
            tipping.compute()
            predicted_y = tipping.output['y']

            error += abs(desired_y - predicted_y)
        return error
